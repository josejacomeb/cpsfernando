...
const size_t bufferSizeesp_cam = JSON_OBJECT_SIZE(11);
DynamicJsonBuffer Bufferentradaesp_cam(bufferSizeesp_cam);
DynamicJsonBuffer Buffersalidaesp_cam(bufferSizeesp_cam);
JsonObject& datoenvioarduinoesp_cam = Buffersalidaesp_cam.createObject();
std::string cadenapararduinoesp_cam;
    ...
    while (1) {
        ...
        if (sensorpresencia) {
		  ...
		  map.insert(std::make_pair("modofunc",
                                sio::bool_message::create(resultado)));
		 vectortotal.push_back(mensajeunico);
                current_socket->emit("datos", mensajetotal);
                vectortotal.clear();
            ...
            if (datojson.success()) {
                ...
                for (auto kv : datojson) {
                    ...
                    if (modofuncionamientoesp) {
                         datoenvioarduino["modofunc"] = true;                    

                        if (strcmp(kv.key, ("estado")) == 0) {
                            map.insert(std::make_pair("estado",
                                    sio::int_message::create(kv.value.as<signed int>())));
                        datoenvioarduino["estado"] = (kv.value.as<signed int>());
                       
                        std::cout << "Cadena para Equipo 1 motor auto esp: ";
                        cadenapararduino = "";
                        ...
                        datoenvioarduinoesp1["modofunc"] = false;
			datoenvioarduinoesp1["umbral"] = 491;
			datoenvioarduinoesp2["modofunc"] = false;
			datoenvioarduinoesp2["umbral"] = 491;
			datoenvioarduinoesp_cam["modofunc"] = false;
			datoenvioarduinoesp_cam["umbral"] = 491;

                        if (strcmp(kv.key, ("sentidomotor")) == 0) {
                            map.insert(std::make_pair("sentidomotor",
                                    sio::int_message::create(kv.value.as<signed int>())));
                            datoenvioarduinoesp1["sentidomotor"] = (kv.value.as<signed int>());      
			
                        std::cout << "Cadena para Equipo 1 manual esp8266: ";
                        cadenapararduinoesp1 = "";

                        datoenvioarduinoesp1.printTo(cadenapararduinoesp1);
                        // cadenapararduino += std::string("\0"); //Fin de cadena
                        ...
                        cadenapararduinoesp2 = "";
                         ...
                         if (strcmp(kv.key, ("capturarengrane")) == 0) {
                            map.insert(std::make_pair("capturarengrane",
                                    sio::bool_message::create(kv.value.as<bool>())));
                              	datoenvioarduinoesp_cam["capturarengrane"] = (kv.value.as<bool>());
                                   bool capturarengraneesp= (kv.value.as<bool>());
                                std::cout << "Cadena para Equipo 1 manual esp8266: ";
	                       
				cadenapararduinoesp_cam = "";
	                        datoenvioarduinoesp_cam.printTo(cadenapararduinoesp_cam);
	                        // cadenapararduino += std::string("\0"); //Fin de cadena
	                        const char *cstr = cadenapararduinoesp_cam.c_str();
	                        std::cout << cstr << std::endl;
	                        serialPuts(fd, cstr); //Envio que realizo el ESP8622
                       		cadenapararduinoesp_cam = "";
